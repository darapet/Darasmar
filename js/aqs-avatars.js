...
(function(window){
  ...
  function showScoreboardSpeech(pos,text,duration,charId){
    ...
    /* Show a floating character popup on screen */
    var ch = charId ? getChar(charId) : null;
    $('#aqs-char-popup').remove();
    var imgHtml = ch ? _charImgEl(ch, 70, 96, 'aqs-char-popup-img') : '';
    var $popup = $('<div id="aqs-char-popup" class="aqs-char-popup-wrap"></div>');
    $popup.html(
      '<div class="aqs-char-popup-inner">' +
        (imgHtml ? '<div class="aqs-char-popup-portrait">'+imgHtml+'</div>' : '') +
        '<div class="aqs-char-popup-bubble">'+$('<span>').text(text).html()+'</div>' +
      '</div>'
    );
    $('body').append($popup);
    /* Also attach speech bubble to scoreboard row for context */
    var $row=$('#aqs-ch-sp-'+pos);
    if($row.length){
      $row.css('position','relative').find('.aqs-char-speech').remove();
      var $b=$('<div class="aqs-char-speech"></div>').text(text);
      $row.append($b);
      var ms=duration||2600;
      setTimeout(function(){ $b.addClass('aqs-speech-fade'); setTimeout(function(){$b.remove();},400); },ms);
    }
  ...
  function showAnswerReaction(charId,isCorrect,line){
    var ch=getChar(charId);
    $('#aqs-ans-reaction').remove();
    var text=line||getRandomLine(charId,isCorrect?'correct':'wrong');
    var imgHtml=_charImgEl(ch,80,110,'aqs-ans-react-img'+(isCorrect?'':' wrong-ans'));
    var $wrap=$('<div id="aqs-ans-reaction"></div>');
    $wrap.append(imgHtml);
    $wrap.append('<div class="aqs-ans-react-speech'+(isCorrect?'':' wrong-ans')+'">'+text+'</div>');
    $('body').append($wrap);
    speakLine(text,charId);
    setTimeout(function(){ $wrap.css({opacity:0,transition:'opacity .35s'}); setTimeout(function(){$wrap.remove();},360); },2800);
  }
  ...
  window.AQS_AVATARS={
    ...
    showAnswerReaction:showAnswerReaction, buildResultsReaction:buildResultsReaction,
