/* AQS Challenge Mode v2 — aqs-challenge.js */
  ...
  function updateRoundsPreview(){
      ...
      var totalQuest = perPlayer * n;
      var valid = (perPlayer % 2 === 0) && perPlayer >= 5;
  ...
  function submitAnswer(idx){
      ...
      $.post(AQS.ajax_url,{action:'aqs_ch_answer',nonce:AQS.public_nonce,
          ...
          /* ── Instant visual feedback with comment + explanation ── */
          $('.aqs-ch-wait-msg').remove();
          var _myCharId = CH.myCharId || localStorage.getItem('aqs_ch_char') || 'blaze';
          if(correct){
              playCorrect();
              if(window.AQS_AVATARS){
                  AQS_AVATARS.triggerDance(CH.position);
                  AQS_AVATARS.showAnswerReaction(_myCharId, true);
              }
              ...
              fbHtml+='<strong>✅ Correct! Well done!</strong>'+_boostLabel;
              if(explanation) fbHtml+='<div class="aqs-fb-expl">💡 '+chRenderMath(explanation)+'</div>';
              fbHtml+='<div class="aqs-fb-next">Moving to next question…</div>';
              fbHtml+='</div>';
              $('#aqs-ch-question-area').append(fbHtml);
          } else {
              playWrong();
              if(window.AQS_AVATARS) AQS_AVATARS.showAnswerReaction(_myCharId, false);
              ...
              if(explanation) fbHtml+='<div class="aqs-fb-expl">💡 '+chRenderMath(explanation)+'</div>';
              fbHtml+='<div class="aqs-fb-next">Moving on…</div>';
              fbHtml+='</div>';
              $('#aqs-ch-question-area').append(fbHtml);
          }
  ...
  function startSyncedCountdown(totalSecs, serverStart){
      ...
      function tick(){
          ...
          if(left<=0){
              stopCountdown();
              /* Auto-submit the pending selection when timer expires */
              if(!CH.answered && CH.selectedAnswerIdx>=0 && !_autoSubmitted){
                  _autoSubmitted=true;
                  submitAnswer(CH.selectedAnswerIdx);
              }
          }
  ...
  function showRevealOverlay(d){
      ...
      /* ── Character dialogue — only YOUR character reacts (no echo) ─── */
      if(window.AQS_AVATARS && AQS_AVATARS.showScoreboardSpeech && CH._playerChars){
          var ansPos   = parseInt(lr.player_pos);   /* who answered */
          var winP     = parseInt(lr.winner_pos);
          var myPos    = CH.position;               /* THIS client's player position */
          var myCharId = CH._playerChars[myPos] || CH.myCharId || 'blaze';

          /* Decide what type of line to pick for this player's character */
          var myLineType;
          if(myPos === ansPos){
              /* This client was the one who answered */
              myLineType = (!skipped && winP === ansPos) ? 'correct' : 'wrong';
          } else {
              /* This client is watching someone else answer */
              if(winP === myPos){
                  myLineType = 'correct';  /* my team / I benefit */
              } else if(winP === ansPos){
                  myLineType = 'defeat';   /* someone else got it right */
              } else {
                  myLineType = 'taunt';    /* answerer also got it wrong */
              }
          }
          var myLine = AQS_AVATARS.getRandomLine(myCharId, myLineType);
          AQS_AVATARS.showScoreboardSpeech(myPos, myLine, 3000, myCharId);
      }
  ...
  async function chGenerateFromTopic(){
      ...
      var totalNeeded=players*perPlayer+3;
  ...
  function chCallAI(prompt){
      /* Generation order:
         1. Groq direct — fastest, best quality (only if key saved in browser)
         2. Pollinations direct — free, no key needed, always available
         3. Server AJAX — last resort, only when proxy URL is configured       */
      var groqKey = typeof window.getGroqKey === 'function' ? window.getGroqKey() : '';
      var step1 = groqKey ? chCallGroqDirect(prompt) : Promise.reject(new Error('no key'));
