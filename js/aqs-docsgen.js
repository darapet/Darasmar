...
(function () {
    ...
    /* Groq direct — uses localStorage key saved via the 🔑 button */
    function callGroq(messages) {
        var key = (typeof window.getGroqKey === 'function'
            ? window.getGroqKey()
            : (cfg.groq_key || '')).trim();
        if (!key) return Promise.reject(new Error('no groq'));
        var ctrl = new AbortController();
        ...
        return fetch('https://api.groq.com/openai/v1/chat/completions', {
            ...
            signal: ctrl.signal,
            body: JSON.stringify({ model: 'llama-3.1-8b-instant', messages: messages, max_tokens: 2000, temperature: 0.5 })
        })
        .then(function (r) { clearTimeout(tid); return r.json(); })
        .then(function (d) {
            var t = (((d.choices || [])[0] || {}).message || {}).content || '';
    ...
    async function raceAI(messages) {
        /* 1. Groq direct — fastest & best quality (key from 🔑 button) */
        try {
            var groqKey = typeof window.getGroqKey === 'function' ? window.getGroqKey() : '';
            if (groqKey) {
