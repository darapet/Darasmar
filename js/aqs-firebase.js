...
async function actionChStart(data) {
    ...
    /* Assign questions to each player.
       Interleave so player 0 gets q[0], q[numPlayers], q[2*numPlayers]...
       and player 1 gets q[1], q[numPlayers+1]...
       This means only qpr*nr total questions are needed (not numPlayers*qpr*nr).
       If somehow the pool is still short, we cycle with modulo so no player ever
       gets an empty list. */
    var assignments = {};
...
async function _advanceQuestion(code, ch) {
    ...
    /* New question — always start with player 0 answering first */
    await update(ref(rtdb, 'challenges/' + code), {
        phase:               'active',
        round:               round,
        q_in_round:          qInRound,
...
async function actionChAnswer(data) {
    ...
    if (!isCorrect) {
        ...
        if (!isStealMode) {
            ...
            for (var si = 1; si <= numPlayers; si++) {
                var checkPos = (pos + si) % numPlayers;
...
async function _advanceNextOrQuestion(code, ch) {
    ...
    if (primaryDone.size >= numPlayers) {
        await _advanceQuestion(code, ch);
        return;
    }
    /* Find next player after current active_pos who still needs primary */
    var start = (ch.active_pos !== undefined ? parseInt(ch.active_pos) : 0);
    var next  = -1;
    for (var i = 1; i <= numPlayers; i++) {
        var check = (start + i) % numPlayers;
